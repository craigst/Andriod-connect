...
def generate_loadsheet(json_data, signature_path=None) -> tuple:
    ...
    extra_notes = "\n".join(car["notes"] for car in cars if car["notes"].strip())
    overall_note = load_summary if not extra_notes else f"{load_summary}\n{extra_notes}"
    ...
    logger.info("Loadsheet generated successfully (XLSX only)")
